...
export const EditButtonModal: React.FC<EditButtonModalProps> = ({ isOpen, onClose, button, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update button label');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
        <div className="bg-white p-6 pb-4 border-b border-gray-100">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Edit Button Label</h2>
              <p className="text-xs text-gray-500 mt-1">Key: {button.key}</p>
            </div>
